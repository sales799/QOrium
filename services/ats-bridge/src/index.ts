...
export async function start() {
  ...
  const server = app.listen(config.port, () => {
    logger.info({ port: config.port }, 'qorium-ats-bridge listening');
  });

  const shutdown = async (signal: string): Promise<void> => {
    logger.info({ signal }, 'shutting down qorium-ats-bridge');
...
// helpers, never this entry point.
{
  void start().catch((err) => {
    process.stderr.write(JSON.stringify({ event: 'atsbridge.fatal', error: String(err) }) + '\n');
    process.exit(1);
  });
}
