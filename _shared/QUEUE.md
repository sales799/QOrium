...
| Deploy/verify Phase 4 observability route | DONE | Active production origin `qorium-active-origin` (`187.127.155.150`) serves `/v1/observability/sentry` with HTTP `200` JSON; current release symlink is `cf717778541b`, repo checkout head is `4999d3a`. | Keep the newer active-origin release; do not roll back to older phase branch. |
