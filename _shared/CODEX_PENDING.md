...
- [ ] QG-05: Production deploy gates.
  ...
  - Live parity proof: current public health is `service=qorium-readybank`, `git_sha=unknown`, `checks.db=not-configured`; current forced VPS-origin health is `service=qorium-uptime-monitor` with missing `git_sha`/`checks.db`, so the new parity gate fails until Cloudflare and Nginx route `/healthz` to the same DB-backed deployment.
  - Live origin proof: normal `https://api.qorium.online/healthz` returns `service=qorium-readybank` with `checks.db=not-configured`; forced VPS origin returns `service=qorium-uptime-monitor`; the normal public request does not appear in the VPS Nginx access log.
  ...
  - Remaining blockers: public Cloudflare origin/tunnel is not routing normal `api.qorium.online` traffic to this VPS; QOrium must be added to the Talpro Rakshak allow-list before a real Rakshak score can be generated.
