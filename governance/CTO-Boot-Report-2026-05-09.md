...
**Fix 1 — Server Actions header mismatch (two passes).** PM2 logs showed `x-forwarded-host header with value 'qorium_admin' does not match origin header with value 'admin.qorium.online'`.

First pass (E.1): added `proxy_set_header X-Forwarded-Host $host;` to the http-context block at the top of `qorium.conf`. `nginx -t` PASS, reloaded; `GET /admin` returned 307 → `/login` rendering cleanly. Looked fixed.

But a fresh `POST /login` test returned 500 with the same digest. **Root cause:** nginx merging — *any* `proxy_set_header` in an inner context drops *all* inherited headers from outer contexts. The `admin.qorium.online` location block had `proxy_set_header Upgrade` + `proxy_set_header Connection` for WebSocket upgrade, which silently overrode the global header set, including `Host` and the X-Forwarded-* family. So none of the standard headers reached the upstream — the X-Forwarded-Host fix at http context never took effect inside the location block.

Second pass (F): rewrote `qorium.conf` so each Next.js subdomain location block (admin/docs/candidate/my) inlines the *complete* proxy_set_header set — `Host`, `X-Real-IP`, `X-Forwarded-For`, `X-Forwarded-Proto`, `X-Forwarded-Host`, `Upgrade`, `Connection` — matching the working pattern in `qorium-marketing.conf`. Backup at `/tmp/qorium.conf.before-headers-fix.20260509-083402`; `nginx -t` PASS; reload. After fix, `POST /login` (the previously-failing Server Action path) returns **200**, no digest in the body. `GET /` no-cache returns 200.
