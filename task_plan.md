...
| --- | --- | --- | --- |
| Deploy current SAML/redirect branch | DONE | Active origin `qorium-active-origin` deployed branch `codex/saml-live-active-origin-20260602` at `031883a26b9d2ddce41a1711340b095d4bc1d9dc`; release `/opt/apps/qorium-marketing/releases/031883a26b9d`; `current` symlink flipped to that release. | Keep live release; rerun deploy only after reviewed changes or DNS update. |
| Verify SAML public surface | DONE | Metadata route returned HTTP `200` with `application/samlmetadata+xml`; login route returned HTTP `302` to `https://www.samltest.dev/...` with `x-qorium-saml-request-id`. | Configure customer IdP metadata/secrets when a tenant arrives. |
...
| Merge PR #88 to `main` | BLOCKED | PR #88 is open/mergeable at `031883a26b9d2ddce41a1711340b095d4bc1d9dc`; production is already deployed from the branch. | Non-author reviewer must review/merge; author must not approve own merge. |
