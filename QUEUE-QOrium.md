...
- [2026-06-03] **Verified local gates** — focused tenant tests passed `5/5`; direct `@qorium/auth`, `@qorium/api`, and `@qorium/web` typechecks passed; full `pnpm typecheck`, `pnpm test` (`5` files / `9` tests), `pnpm scan:secrets` (`268` tracked/untracked text files), `pnpm lint` (`8/8`), `pnpm build` (`1198/1198`), `pnpm smoke`, `pnpm e2e` (`1` Playwright test), and `git diff --check` passed.
