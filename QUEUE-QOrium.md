...
- Existing unrelated workspace changes and untracked generated/business documents were left unstaged. The only app-file diff intentionally left unstaged is `qorium-app/apps/web/package.json` (`lint` script changed from `next lint || true` to `tsc -p tsconfig.json --noEmit`), because it was not part of this closeout.
