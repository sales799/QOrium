...
function _buildReviewSheet(ss) {
  ...
  sheet.getRange(1, C.DECISION, 1, 5).setBackground('#b45309').setFontColor('#ffffff');
  ...
  for (var i = 0; i < PAGES.length; i++) {
    ...
    sheet
      ...
      .setHorizontalAlignment('center')
      .setFontColor('#2563eb');
    sheet.getRange(row, C.STATUS).setValue('⏳ Awaiting Review').setHorizontalAlignment('center');
  ...
  [C.VISUAL, C.CLARITY, C.NAV, C.MOBILE, C.SPEED, C.OVERALL].forEach(function (col) {
    sheet
      .getRange(DATA_START, col, PAGES.length, 1)
  ...
  sheet
    .getRange(DATA_START, C.DECISION, PAGES.length, 1)
    .setDataValidation(
...
function _buildHowToSheet(ss) {
  ...
  sheet.getRange(lastRow, 1, 1, 2).merge().setBackground('#fef3c7').setFontWeight('bold');
...
function _emailCTO_NewReview(sheet, row, name) {
  ...
  var body =
    ...
    '\n' +
    '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n' +
    '→ Open sheet to respond: ' +
    ssUrl +
    '\n\n' +
    'Fill Column O (Decision) + Column P (Notes) + Column Q (Response to Team).\n' +
    'As soon as you fill Column Q, an email goes out to ' +
    name +
    ' automatically.\n\n' +
...
function _emailCTO_ChatQuestion(name, question, row, sheet) {
  ...
  var body =
    name +
    ' posted a question in the Chat sheet (row ' +
    row +
    '):\n\n' +
    '"' +
    question +
    ...
    ssUrl +
    '\n' +
    "Go to tab '💬 Chat with CTO' → Row " +
    row +
    ' → fill Column E (CTO Response).\n' +
