...
- **ReadyBank API** uses it to place customer queries in the right subtree. A platform integration can request "Senior Spring backend questions, band 4-5, 30 items" and the API walks the graph deterministically. No fuzzy LLM tagging in the request path; that's the point.
...
- **Stack-Vault** customers get a _private overlay_ — their own subtree forks where the customer's tech stack diverges from canonical (e.g., Bosch's embedded automotive stack adds nodes that aren't in the shared graph). The overlay is exclusive per Constitution SO-10; it doesn't appear in any other Stack-Vault or in shared ReadyBank.
