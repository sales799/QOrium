...
SAML tokens remain incompatible with the password JWT API gate; this guard is a prerequisite, not working SSO certification. Shared issuance/verifier integration and a real IdP/browser flow are still required. Existing production deployments without SAML database configuration will intentionally receive503 and need verified configuration before SAML can be enabled.
