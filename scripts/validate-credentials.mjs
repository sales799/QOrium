...
async function main() {
  ...
  if (asJson) {
    process.stdout.write(`${JSON.stringify({ strict, failCount, skipCount, results }, null, 2)}\n`);
  } else {
    for (const row of results) {
      process.stdout.write(`${row.status.padEnd(4)} ${row.name.padEnd(26)} ${row.detail}\n`);
    }
    process.stdout.write(
      `\nSummary: ${failCount} fail, ${skipCount} skipped/missing (${strict ? 'strict' : 'non-strict'} mode)\n`,
    );
  }
